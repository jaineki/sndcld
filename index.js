...
app.get('/api/download', async (req, res) => {
    ...
    try {
        // If title/artist provided, check cache first
        if (title) {
                ...
                if (exists) {
                    ...
                    return res.json({ 
                        ...
                        result: { 
                            ...
                            author: cachedData.artist, 
                            downloadUrl: downloadUrl,
                            thumbnail: cachedData.artwork || ''
                        } 
                    });
                }
            }
        ...
        if (response.data.success && response.data.result && title) {
            ...
            const songKey = getSongKey(title, artist || result.author);
            cacheMp3InStorage(songKey, title, artist || result.author, link).catch(e => console.error("BG Cache Error:", e.message));
        }
...
app.get('/api/admin/songs', isAdmin, async (req, res) => {
    try {
        const snapshot = await rtdb.ref('cachedSongs').get();
        const songs = [];
        if (snapshot.exists()) {
...
app.delete('/api/admin/songs/:id', isAdmin, async (req, res) => {
    const { id } = req.params;
    try {
        const snapshot = await rtdb.ref(`cachedSongs/${id}`).get();
        if (snapshot.exists()) {
            const data = snapshot.val();
            if (data.storagePath) {
                await bucket.file(data.storagePath).delete().catch(() => {});
            }
            await rtdb.ref(`cachedSongs/${id}`).remove();
        }
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
