...
// Express matches routes in order. If /api/user/:collection
// is registered before /api/user/playlists, it will match
// "playlists" as the :collection parameter and the dedicated
// playlist endpoints will NEVER be reached.
// ============================================================
...
// --- PLAYLIST ROUTES (REGISTERED FIRST - specific paths) ---
...
app.post('/api/user/playlists/add-song', verifyToken, async (req, res) => {
    ...
    if (!playlistId) {
        return res.status(400).json({ error: 'Playlist ID is required', success: false });
    }
    if (!song) {
        return res.status(400).json({ error: 'Song data is required', success: false });
    }
...
// --- USER COLLECTION ROUTES (REGISTERED LAST - generic paths) ---
// These catch anything under /api/user/:collection that isn't matched above
