...
app.post('/api/user/playlists/create', verifyToken, async (req, res) => {
    ...
    try {
        ...
        console.log(`Playlist created: ${name} (id: ${docRef.id}) for user: ${uid}`);
        res.json({ 
            success: true, 
            id: docRef.id,
            playlist: {
...
app.post('/api/user/playlists/add-song', verifyToken, async (req, res) => {
    ...
    try {
        ...
        // Add song to the playlist's songs array
        const songData = {
        ...
        await playlistRef.update({
            songs: admin.firestore.FieldValue.arrayUnion(songData)
        });
        
        console.log(`Song added to playlist: ${playlistId} for user: ${uid}`);
...
// Get all playlists for a user (explicit endpoint)
...
app.post('/api/user/playlists/remove-song', verifyToken, async (req, res) => {
    const { playlistId, songUrl } = req.body;
    const uid = req.user.uid;
    try {
        const playlistRef = db.collection('users').doc(uid).collection('playlists').doc(playlistId);
        const playlistDoc = await playlistRef.get();
        ...
        const updatedSongs = (playlistData.songs || []).filter(s => s.url !== songUrl);
